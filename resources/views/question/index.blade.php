	    ...
	    <div class="card">
	    	...
	    	<div class="card-body p-2">
	    			...
	    			<div class="col-md-4">
	    				<form id="form" method="get" action="{{ url('questions') }}">
		    				<div class="form-group">
						      <select onchange="getQuestions()" class="form-control" name="subject_id" class="subject_id" id="subject_id">
	    		...
	    		<form id="create_exam_paper_form" method="post" action="{{ route('exam-papers.store') }}">
	    			@csrf
	    			<input type="hidden" id="input_subject_id" name="subject_id" value="{{ $subject_id }}">
...
<script type="text/javascript">
	//getting questions without api
	function getQuestion(){
		$('#form').submit();
	}

	//getting questions with api
	function getQuestions(){
		$.ajax({
			url: '{{ url('api/questions') }}/'+$('#subject_id').val(),
			type: 'GET',
			dataType: 'json',
		})
		.done(function(response) {
			if(response){
				...
				$.each(response, function(index, val) {
					 data += '<tr id="tr-'+val.id+'">'+
                        '<td class="text-center"><input style="width: 17px !important; height: 17px !important" type="checkbox" name="selected_question[]" value="'+val.id+'"></td>'+
                        '<td>'+val.id+'</td>'+
                        '<td>'+val.subject.name+'</td>'+
                        '<td>'+val.question+'</td>'+
                        '<td class="text-center">'+
                            '<button type="button" onclick="editPage(';
                            data+= "'"+route+"'";
                            data+=','+val.id+',';
                            data+= "'"+name+"'";
                            data+= ')" class="btn btn-sm btn-primary"><i class="fas fa-edit"></i></button>&nbsp;'+
                            '<button type="button" onclick="Delete(';
                            data+= "'"+route+"'";
                            data+= ','+val.id+')" class="btn btn-sm btn-danger"><i class="fas fa-trash"></i></button>'+
                        '</td>'+
                    '</tr>'
				});

				$('#questions').html(data);
				$('#input_subject_id').val($('#subject_id').val());
			}
		});
	...
	function searchQuestion() {
	  ...
	  for (i = 1; i < tr.length; i++) { // except first(heading) row
	  	count_td = 0;
	  	for(j = 1; j < column_length-1; j++){ // except first and last column 
